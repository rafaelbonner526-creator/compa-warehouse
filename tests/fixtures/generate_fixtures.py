...
Every column a model references carries at least one non-null value, because
dlt drops all-null columns on load (no values -> no inferable type).
...
for i in range(1, 11):
    leads.append({
        ...
        "content_signals": "blog, newsletter",
        "linkedin_url": f"https://linkedin.test/in/lead{i}",
        "linkedin_active": ["true","false","yes",""][i % 4],
        "community_platform": "circle", "audience_size_estimate": "medium",
...
objections = ["timing","budget","not_now"]
touches = []
tid = 0
for lead_id in range(1, 11):
    for n in range(1, (lead_id % 3) + 1):  # 1-2 touches per lead
        ...
        touches.append({
            ...
            "reply_sentiment": sentiments[tid % 3] if replied else "",
            "objection_tag": objections[tid % 3] if tid % 4 == 0 else "",
